...
Parser.prototype = {
  ...
  object: function(){
    ...
    while (!this.accept('}')) {
      ...
      id = this.accept('ident') || this.string();
      if (!id) this.error('expected "ident" or "string", got {peek}');
      id = id.val ? id.val.hash : id.string;
      this.skipSpacesAndComments();
      this.expect(':');
      val = this.expression();
      obj.set(id, val);
  ...
  string: function() {
    var quote;
    if (quote = this.accept('"') || this.accept("'")) {
      var segs = []
        , tok, prev;
      while (!~[quote.type, 'newline', 'eos'].indexOf(this.peek().type)) {
        tok = this.peek();
        if (tok.type != '{' || this.lookahead(2).type == quote.type) {
          segs.push(new nodes.Literal(this.next().val));
        } else {
          prev = this.lexer.prev.val;
          if (prev && prev.val
            && '\\' == prev.val[prev.val.length - 1]) {
            segs.push(new nodes.Literal(this.next().val));
          } else {
            segs = segs.concat(this.interpolate());
          }
        }
      }
      this.expect(quote.type);
      this.skipSpaces();
      return new nodes.String(segs, quote.type);
    }
  },
