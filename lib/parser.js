...
var Parser = module.exports = function Parser(str, options) {
  ...
  this.paths = options.paths || [dirname(this.filename)];
};
...
Parser.prototype = {
  ...
  get import() {
    this.expect('import');
    var css
      , path = this.expect('string').val.val;
    ...
    for (var i = 0, len = this.paths.length; i < len; ++i) {
      var lookup = this.paths[i] + '/' + path;
      try {
        css = fs.readFileSync(lookup, 'utf8');
        path = lookup;
        break;
      } catch (err) {
        // Ignore
    ...
    if (!css) throw new Error('failed to import ' + path);

    return new nodes.String(css);
  },
