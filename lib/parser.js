...
Parser.prototype = {
  ...
  looksLikeSelector: function(fromProperty) {
    ...
    // Nested property $p.color property
    if (
      fromProperty
    ) {
      let j = i + 1, node;
      while((node = this.lookahead(j)) && node.type !== 'newline' && node.type !== 'eos') {
        if (
          'ident' == node.type &&
          node.val && node.val.name &&node.val.name.indexOf('$') === 0 &&
          '.' == this.lookahead(j + 1).type &&
          'ident' == this.lookahead(j + 2).type
        ) {
          return false;
        }
        j += 1;
