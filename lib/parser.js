...
Parser.prototype = {
  ...
  get for() {
    this.expect('for');
    var key
      , val = this.expect('ident').val.name;
    if (this.accept(',')) key = this.expect('ident').val.name;
    this.expect('in');
  ...
  get keyframes() {
    ...
    this.expect('indent');
    while ('unit' == this.peek.type) {
      this.state.push('keyframe');
      var pos = this.expect('unit').val
        , block = this.block(keyframes);
