...
var Parser = module.exports = function Parser(str, options) {
  ...
  this.filename = nodes.filename = options.filename || 'stylus';
...
Parser.prototype = {
  ...
  get unless() {
    ...
    node.ifBlock = this.block(node, false);
    this.state.pop();
    return node;
  },
  ...
  get if() {
    ...
    node.ifBlock = this.block(node, false);
    if (this.accept('else')) {
      node.elseBlock = this.block(node, false);
    }
    this.state.pop();
    return node;
  },
  ...
  get property() {
    ...
    // indicating that this is really
    // a selector
    var i = 0;
    ...
    prop.expr = this.list;
    if (prop.expr.isEmpty) return ident;
    this.state.pop();
  ...
  get negate() {
    if (this.accept('not')) {
      return new nodes.UnaryOp('!', this.negate);
    }
    return this.ternary;
  },
  ...
  get primary() {
    ...
    if (op =
      ...
      || this.accept('-')) {
      this.operand = true;
      node = new nodes.UnaryOp(op.type, this.primary);
      this.operand = false;
      return node;
    }
