...
Parser.prototype = {
  ...
  get property() {
    ...
    if (prop.expr.isEmpty) throw new Error('property "' + name + '" expects an expression, got "' + this.peek.type + '"');
    this.lexer.isSelector = true;
  ...
  get variable() {
    ...
    if (this.accept('assign')) {
      ...
      if (expr.isEmpty) throw new Error('invalid right-side operand in assignment, got "' + this.peek.type + '"');
  ...
  get expression() {
    ...
    // TODO: Expression
    while (node = this.term) expr.push(node); 
    return expr;
  },
