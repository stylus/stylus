...
Parser.prototype = {
  ...
  get functionCall() {
    ...
    this.expect(')');
    // TODO: fix me im a hack, .expression should
    // always be an Expression node
    if (!(args.nodes[0] instanceof nodes.Expression)) {
      var expr = new nodes.Expression;
      if (args.nodes.length) expr.push(args.nodes[0]);
      args.nodes[0] = expr;
    }
    return new nodes.Call(name, args.nodes[0]);
  },
  ...
  get params() {
    ...
    while (tok = this.accept('variable')) {
      params.push(tok.val);
      if (this.accept('assign')) {
        tok.val.val = this.additive || new nodes.Null;
      }
      this.accept(',');
    }
    return params;
