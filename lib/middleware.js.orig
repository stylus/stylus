  ...
  return function(req, res, next){
    ...
    if (/\.css$/.test(path)) {
      var cssPath = join(dest, path)
        ...
        , jsPath = join(src, path.replace('.css', '.support.js'));
      ...
      function compile() {
        ...
        fs.readFile(stylusPath, 'utf8', function(err, str){
          ...
          style.render(function(err, css, js){
...
<<<<<<< HEAD
  nodes.forEach(function(_import){
    fs.stat(_import.path, function(err, stat){
      // error or newer mtime
      if (err || !_import.mtime || stat.mtime > _import.mtime) {
        changed.push(_import.path);
=======
...
>>>>>>> e824692... replaced reserved keyword `import` with `imported`
      }
      --pending || fn(changed);
    });
  });
