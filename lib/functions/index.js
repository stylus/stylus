...
exports.red = function red(color, value){
  if (value) {
    ...
    if (value.type == '%') {
      ...
      throw new Error('TypeError: expected "value" to be a number, but got "' + value.toString() + '"');
    }
    return new nodes.RGBA(
      value.val,
      color.g,
...
exports.green = function green(color, value){
  if (value) {
    ...
    if (value.type == '%') {
      ...
      throw new Error('TypeError: expected "value" to be a number, but got "' + value.raw + '"');
    }
    return new nodes.RGBA(
      color.r,
      value.val,
