...
(exports.substr = function substr(val, start, length){
  ...
  val = utils.unwrap(val).nodes[0].string;
  start = utils.unwrap(start).nodes[0].val;
  if (length) {
    length = utils.unwrap(length).nodes[0].val;
  }
  return new nodes.Literal(val.substr(start, length));
}).raw = true;
...
(exports.replace = function replace(pattern, replacement, val){
  ...
  val = utils.unwrap(val).nodes[0].string;
  return new nodes.Literal(val.replace(pattern, replacement));
}).raw = true;
...
(exports.split = function split(delim, val){
  ...
  val = utils.unwrap(val).nodes[0].string;
  var splitted = val.split(delim);
  var expr = new nodes.Expression();
  for (var i = 0, len = splitted.length; i < len; ++i) {
    expr.nodes.push(new nodes.Literal(splitted[i]));
  }
  return expr;
}).raw = true;
