...
/**
 * Convert a .json file into stylus object.
 *
 * Examples:
 *
 *    vars = json('vars.json')
...
(exports.merge = exports.extend = function merge(dest){
  ...
  var lastParam = utils.unwrap(arguments[arguments.length - 1]).first;
  var isRecursive =  lastParam.val == true;

  for (var i = 1, len = arguments.length - isRecursive; i < len; ++i) {
    utils.merge(dest.vals, utils.unwrap(arguments[i]).first.vals, isRecursive);
  }
  return dest;
}).raw = true;
