...
exports.contrast = function contrast(top, bottom){
  ...
  function contrast(top, bottom) {
    ...
    var l2 = exports.luminosity(top).val + 0.05;
    var ratio = l1 / l2;
  ...
  if (bottom.a >= 1) {
    ...
    var onWhite = contrast(top, exports.blend(bottom, new nodes.RGBA(255, 255, 255, 1)));
 ...
 * @param {RGBA|HSLA} top
 * @param {RGBA|HSLA} [bottom=#fff]
 * @param {Unit} [minAlpha=0]
 * @return {RGBA}
 * @api public
 */
exports.transparentify = function transparentify(top, bottom, minAlpha){
  utils.assertColor(top);
  // Handle default arguments
  bottom = bottom || new nodes.RGBA(255, 255, 255, 1);
  if (!minAlpha && bottom && !bottom.rgba) {
    minAlpha = bottom;
    bottom = new nodes.RGBA(255, 255, 255, 1);
  }
  utils.assertColor(bottom);
  // Find the best alpha for the given colors
  var bestAlpha = [
    top.r - bottom.r,
    top.g - bottom.g,
    top.b - bottom.b
  ...
  bestAlpha = Math.abs(bestAlpha) / 255;
  // Adjust the best alpha if there is `minAlpha` in args
  if (minAlpha) {
    utils.assertType(minAlpha, 'unit', 'minAlpha');
    if (minAlpha.type == '%') {
      minAlpha = minAlpha.val / 100;
    } else if (minAlpha.type) {
      minAlpha = 0;
    } else {
      minAlpha = minAlpha.val;
    }
    bestAlpha = Math.max(
      bestAlpha,
      Math.min(minAlpha, 1)
