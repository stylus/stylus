...
/**
 * Test if `expr` contains the given `pattern`, and where. Similar to `match()`, but for expressions. Returns '-1' if no match exists.
 *
 * Examples:
 *
 * @param {String} pattern
...
(exports.contains = function contains(pattern, expr){
  if (typeof pattern.nodes[0].string === 'undefined') var re = new RegExp(pattern.nodes[0].nodes[0].string);
  else var re = new RegExp(pattern.nodes[0].string);
  for (x in utils.unwrap(expr).nodes){
    console.log(utils.unwrap(expr).nodes[x].string);
    if (utils.unwrap(expr).nodes[x].val == 'null' && re.test(utils.unwrap(expr).nodes[x].string)) return utils.coerce(x)
    if (utils.unwrap(expr).nodes[x].val != 'null' && re.test(utils.unwrap(expr).nodes[x].val.toString())) return utils.coerce(x);
  }
  return utils.coerce(-1);
}).raw = true;
