    ...
    if (/\.css$/.test(path)) {
      ...
      function compile() {
        fs.readFile(stylusPath, 'utf8', function(err, str){
          ...
          style.render(function(err, css){
            ...
            if (debug) console.error('    rendered %s', stylusPath);
      ...
      fs.stat(stylusPath, function(err, stylusStats){
        ...
        fs.stat(cssPath, function(err, cssStats){
          ...
          if (err) {
            if (ENOENT == err.errno) {
              if (debug) console.error('    not found %s', cssPath);
              compile();
            } else {
              next(err);
            }
            ...
            if (stylusStats.mtime > cssStats.mtime) {
              if (debug) console.error('    changed %s', cssPath);
              compile();
            // Already compiled, check imports
            } else {
              checkImports(stylusPath, function(changed){
                if (debug && changed) console.error('    import changed %s', changed);
