...
module.exports = function(options){
  ...
  // Serve by default
  var serve = false !== options.serve;

  // Connect reference
  if (serve) send = require('connect').static.send;
  ...
  return function stylus(req, res, next){
    ...
    if (/\.css$/.test(path)) {
      var cssPath = join(dest, serve ? basename(path) : path)
        , stylusPath = join(src, path.replace('.css', '.styl'));

      debug('src %s', stylusPath);
      debug('dst %s', cssPath);
        ...
        fs.readFile(stylusPath, 'utf8', function(err, str){
          ...
          style.render(function(err, css){
            ...
            mkdirp(dirname(cssPath), 0700, function(err){
              ...
              if (serve) {
                var options = {};
                options.path = encodeURIComponent(cssPath);
                debug('to %s', cssPath);
                fs.writeFile(cssPath, css, 'utf8', function(err){
                  if (err) return next(err);
                  debug('serve %s', cssPath);
