    ...
    if (/\.css$/.test(path)) {
      ...
      function compile() {
        ...
        fs.readFile(stylusPath, 'utf8', function(err, str){
          ...
          style.options._importCache = importCache;
          ...
          style.render(function(err, css){
            ...
            paths.forEach(function(imported) {
              var cache = importCache[imported.path];
              if (cache && cache.mtime) {
                imported.mtime = cache.mtime;
              } else {
...
function checkImports(path, fn) {
  ...
  nodes.forEach(function(imported){
    fs.stat(imported.path, function(err, stat){
      ...
      // Invalidate the import cache as a distinct object
      var cache = importCache[imported.path];
      if (cache && !cache.mtime || stat.mtime > cache.mtime) {
        delete importCache[imported.path];
      };
