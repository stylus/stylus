...
Normalizer.prototype.visitMedia = function(media){
  ...
  if (props.length) {
    ...
    mergeQueries(media.block.parent);
    media.bubbled = true;
  }

  if (!media.bubbled && medias.length) {
    var block = new nodes.Block;
    media.block.nodes = media.block.nodes.filter(function(node){
      return 'media' != node.nodeName;
    });
    block.push(media);
    mergeQueries(block);
    return block;
