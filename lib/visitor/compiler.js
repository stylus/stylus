...
Compiler.prototype.visitRoot = function(block){
  ...
  if (this.sourcemaps) {
    var map = this.sourcemapsGenerator.toJSON(),
        rawFilename = require('path').basename(node.filename, '.styl');
    map.sourcesContent = [];
    map.sources.forEach(function(name, idx) {
      map.sourcesContent[idx] = fs.readFileSync(name, 'utf8');
    });
    var sourceMapComment = '/*# sourceMappingURL=' + rawFilename + '.styl.map */';
    fs.writeFileSync(node.filename + '.map', JSON.stringify(map));
    return this.buf + '\n' + sourceMapComment;
  }
  ...
  if (this.sourcemaps) {
    ...
    if (!this.sourcemapsGenerator) {
      var SourceMapGenerator = require('source-map').SourceMapGenerator,
          rawFilename = require('path').basename(node.filename, '.styl');
      this.sourcemapsGenerator = new SourceMapGenerator({file: rawFilename + ".css"});
    }
