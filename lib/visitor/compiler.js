...
Compiler.prototype.visitRoot = function(block){
  ...
  for (var i = 0, len = block.nodes.length; i < len; ++i) {
    ...
    if (this.linenos || this.firebug || this.sourcemaps) this.debugInfo(node);
    var ret = this.visit(node);
    if (ret) this.buf += ret + '\n';
  }

  if (this.sourcemaps) {
    var map = this.sourcemapsGenerator.toJSON();
    map.sourcesContent = [];
    map.sources.forEach(function(name, idx) {
      map.sourcesContent[idx] = require('fs').readFileSync(name, 'utf8');
    });
    var encoded = (new Buffer(JSON.stringify(map))).toString('base64');
    var sourceMapComment = '/*# sourceMappingURL=data:application/json;base64,' + encoded + '*/';
    fs.writeFileSync('bundle.map', JSON.stringify(map));
    return this.buf + '\n' + sourceMapComment;
  }
...
Compiler.prototype.debugInfo = function(node){
  ...
  if (this.sourcemaps) {
    ...
    if (!this.sourcemapsGenerator) {
      var SourceMapGenerator = require('source-map').SourceMapGenerator;
      this.sourcemapsGenerator = new SourceMapGenerator({file: "bundle.css"});
    }
    // TODO: optimise this - track all line additions
    var bufLine = this.buf.split('\n').length;
    if (!line)
      line = 1;
    this.sourcemapsGenerator.addMapping({
      original: {
        line: line,
        column: 0
      },
