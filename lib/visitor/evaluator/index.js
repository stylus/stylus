...
var Evaluator = module.exports = function Evaluator(root, options) {
  ...
  this.stack.push(this.global = new Frame(this.block = root));
};
...
Evaluator.prototype.visitBlock = function(block){
  this.stack.push(new Frame(this.block = block));
  for (var i = 0, len = block.nodes.length; i < len; ++i) {
    block.nodes[i] = this.visit(block.nodes[i]);
  }
  this.stack.pop();
...
Evaluator.prototype.visitCall = function(call){
  // TODO: prevent passing of this.block
  // maintain this in the Stack
  // TODO: use visitVariable() to normalize lookup
  var fn = this.stack.lookup(this.block, call.name);
  ...
  stack.push(new Frame(this.block = fn.body));
...
Evaluator.prototype.visitVariable = function(variable){
  if (this.lookup) {
    var val = this.stack.lookup(this.block, variable.name);
