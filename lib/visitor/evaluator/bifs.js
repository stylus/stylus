...
exports.unit = function(n){
  utils.assertType(n, nodes.Unit);
  return new nodes.String(n.type || '');
};
...
exports.lighten = function(color, amount){
  utils.assertColor(color);
  utils.assertType(amount, nodes.Unit);
  var hsl = color.hsl;
...
/**
 * Convert `unit` to the given `type`.
 *
 * @param {Unit} unit
 * @param {String} type
 * @return {Unit}
...
exports.unit = function(unit, type){
  utils.assertType(unit, nodes.Unit);
  utils.assertType(type, nodes.String);
  return new nodes.Unit(unit.val, type.val);
};