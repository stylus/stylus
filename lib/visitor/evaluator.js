...
Evaluator.prototype.visitRequire = function(imported) {
  ...
  // url() passed
  if ('url' == path.name) throw new Error('You cannot @require a url');

  // Ensure string
  if (!path.string) throw new Error('The path of a @require should be a string');
  var name = path = path.string;
  ...
  // Literal
  literal = ~path.indexOf('.css') && !~path.indexOf('.css.');
  ...
  // Throw if import failed
  if (!found) throw new Error('failed to locate @require file ' + path);

  if (!~this.requireHistory.indexOf(found)) {
    this.requireHistory.push(found);
    return this.visitImport(imported);
  }
