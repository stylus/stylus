...
Evaluator.prototype.visitImport = function( _import){
  var found
    , root = this.root
    , Parser = require('../parser')
    , path = this.visit(_import.path).first;
  ...
  // Literal
  if (/\.css$/.test(path)) return _import;
  path += '.styl';
  ...
  // Expose imports
  _import.path = found;
  _import.dirname = dirname(found);
  this.paths.push(_import.dirname);
  if (this.options.imports) this.options.imports.push(_import);
