...
Evaluator.prototype.visitViewport = function(viewport){
  if (viewport.fabricated) return viewport;
  viewport.block = this.visit(viewport.block);

  if ('official' != viewport.prefix) return viewport;

  this.vendors.forEach(function(prefix){
    var node = viewport.clone();
    node.prefix = prefix;
