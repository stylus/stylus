...
Evaluator.prototype.evaluate = function(fn){
  try {
    ...
    this.setup();
    return this.visit(this.root, fn);
  } catch(err) {
    if(!fn) throw err;
    fn(err);
  }
...
Evaluator.prototype.visitImport = function(importNode, fn){
  var fnSync = fn || function(err, result) {
    if(err) throw err;
    return result;
  }
  var self = this;
  ...
  var root = self.root
    ...
    , path = self.visit(importNode.path).first
    , importer;
  ...
  // url() passed
  if ('url' == path.name) return fnSync(null, importNode);

  // Enusre string
  if (!path.string) return fnSync(new Error('@import string expected'));
  var name = path = path.string;

  importer = new self.options.Importer(path, this.filename, self.options);

  if(fn) importer.lookup(lookedup);
  else return lookedup(null, importer.lookupSync());
  function lookedup(err, found) {
    if(err) return fnSync(err);

    if(!found) return fnSync(null, importNode);

    // Expose imports
    importNode.path = found;
    importNode.dirname = dirname(found);
    // self.paths.push(importNode.dirname);
    if (self.options._imports) self.options._imports.push(importNode);

    // Parse the file
    self.importStack.push(found);
    nodes.filename = found;

    if(fn) importer.import(imported);
    else return imported(null, importer.importSync());
    function imported(err, result) {
      if(err) return fnSync(err);

      var str = result.str;
      if (result.literal) return fnSync(null, new nodes.Literal(str.replace(/\r\n?/g, "\n")));
      ...
      importNode.mtime = result.mtime;

      // Evaluate imported "root"
      block.parent = root;
      block.scope = false;
      var oldFilename = self.filename;
      self.filename = found;
      if(fn) self.visit(block, visited);
      else return visited(null, self.visit(block));
      function visited(err, ret) {
        if(err) return fnSync(err);
        self.filename = oldFilename;
        self.importStack.pop();
        return fnSync(null, ret);
      }
    }
  }
};
...
Evaluator.prototype.invoke = function(body, stack, filename){
  ...
  if(filename) this.filename = oldFilename;
