...
Evaluator.prototype.visitString = function(node){
  node.val = this.interpolate(node);
  return node;
};
...
Evaluator.prototype.visitCharset = function(node){
  this.return++;
  node.val = this.visit(node.val);
  this.return--;
  return node;
};

Evaluator.prototype.visitNamespace = Evaluator.prototype.visitCharset;
...
Evaluator.prototype.visitQuery = function(node){
  if (node.predicate) node.predicate = this.visit(node.predicate);
