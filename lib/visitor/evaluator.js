...
Evaluator.prototype.visitMember = function(node){
  var left = node.left
    ...
    , err = new Error(left + ' has no property .' + right)
    , val;

  if ('object' != obj.nodeName) throw err;
  if (node.val) {
    this.return++;
    obj.set(right.name, this.visit(node.val));
    this.return--;
  }
  val = obj.get(right.name);
  if (val.isNull) throw err;
  return val;
};
...
Evaluator.prototype.visitIdent = function(ident){
  ...
  // Lookup
    ...
    // Undefined strict variable
    if (!val && ident.isStrict)
      throw new Error('variable ' + ident.name + ' is undefined');
