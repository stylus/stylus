...
Evaluator.prototype.visitImport = function(imported){
  ...
  // Lookup
  var find = (function () {
    var found = utils.find(path, this.paths, this.filename);
    if (!found) {
  ...
  if (this.options.importResolver && !utils.absolute(path)) {
    found = this.options.importResolver(name, imported.filename, find);
  } else {
    found = find();
  }
