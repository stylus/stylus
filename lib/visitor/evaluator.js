 ...
 * Visit Content.
 */
Evaluator.prototype.visitContent = function(content) {
  /**
   * Todo: Find a better way to get the content block
   *
   * Bubble up the parents until we find a node that
   * has an attached .content property
   */
  var currentNode = content.parent
    , contentBlock;

  out:
  while (currentNode = currentNode.parent) {
    if (contentBlock = currentNode.content) break out;
  }

  if (!contentBlock) throw new Error('The called function expects content')

  return this.invoke(contentBlock);
};
...
Evaluator.prototype.visitCall = function(call){
  ...
  // User-defined
    ...
    ret = this.invokeFunction(fn, args, contentBlock);
  }
...
Evaluator.prototype.invokeFunction = function(fn, args, contentBlock){
