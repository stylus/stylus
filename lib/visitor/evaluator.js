 ...
 * @api private
 */
function importFile(node, file, literal, content) {
...
Evaluator.prototype.visitImport = function(imported){
  ...
  // try locating file with custom importer
  var contents = [];
  if (!found) {
    if (this.options.importer) {
      found = this.options.importer.find(path, this.paths, this.filename);
      if (found) {
        contents = found.map(this.options.importer.readFile);
      }
    }
  }
  ...
  for (var i = 0, len = found.length; i < len; ++i) {
    block.push(importFile.call(this, imported, found[i], literal, contents[i]));
  }
