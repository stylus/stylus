...
Evaluator.prototype.visitImport = function(imported){
  ...
  found = utils.lookup(path, this.paths, this.filename) ||
          utils.lookup(join(name, 'index.styl'), this.paths, this.filename);
  ...
  // Nested imports
  if (importStack.length) {
    var last = importStack[importStack.length - 1];
    this.paths.push(dirname(last), imported.dirname);
  }
  ...
  // Store importStack/paths stack size
  var importStackLen = importStack.length;
  var pathsLen = this.paths.length;

  var ret = this.visit(block);

  // Pop until we return to the previous stack size
  while (importStackLen <= importStack.length) importStack.pop();
  while (pathsLen <= this.paths.length) this.paths.pop();
