...
Evaluator.prototype.__defineGetter__('currentSelector', function(){
  ...
  for (var i = 0; i < len; ++i) {
    ...
    if (block.node && 'group' == block.node.nodeName) {
      prev = selectors[selectors.length - 1];
      selector = block.node.selector;
      if (selector && ~selector.indexOf('&')) {
        selectors[selectors.length - 1] = selector.replace(/&/g, prev).trim();
      } else {
        selectors.push(selector);
      }
    }
  }
  if (selectors.length) return selectors.join(' ');
});
