...
Lexer.prototype = {
  ...
  strchars: function() {
    var captures
      , quote = this.qstack[this.qstack.length - 1];
    if (!this.inQuotes || !quote) return;

    var rx = new RegExp('^[^' + quote + '{]+');
    if (captures = rx.exec(this.str)) {
      this.skip(captures);
      return new Token('literal', new nodes.Literal(captures[0]));
    }
  },
  ...
  space: function() {
    ...
    if (captures = /^([ \t]+)/.exec(this.str)) {
      this.skip(captures);
      return new Token('space', captures[0]);
    }
  },
  ...
  brace: function() {
    ...
    if (captures = /^([{}])/.exec(this.str)) {
      ...
      this.inQuotes = ('}' == brace && this.qstack.length);
  ...
  quotes: function() {
    if ("'" == this.str[0] || '"' == this.str[0]) {
      ...
      this.inQuotes = !!stack.length;
      return new Token(quote, quote);
    }
  },
