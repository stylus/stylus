...
function Lexer(str, options) {
  ...
  str = str
    ...
    .replace(/\\ *\n/g, '\r')
    .replace(/([,(:](?!\/\/[^ ])) *(?:\/\/[^\n]*|\/\*.*?\*\/)?\n\s*/g, comment)
    // NOTE: the following pattern still backtracks when \S doesn't match. Make sure to trimEnd() first.
    .replace(/\s*(\S)/g, (s, val, offset, whole) => {
      const iN = s.lastIndexOf("\n");
      if (iN > -1 && /^[ \t]*[,)]/.test(s.slice(iN + 1))) {
        return comment(s, val, offset, whole);
      }
      return s;
    });
  this.str = str;
