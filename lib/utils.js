...
/**
 * Coerce a javascript object to a Stylus `Object`.
 *
 * @param {Object} obj
 * @return {Object}
 * @api public
...
/**
 * Merge object `b` with `a`. If recursive flag is set true, merges inner hashes too
 *
 * @param {Object} a
 * @param {Object} b
 ...
 * @api private
 */
exports.merge = function(a, b, isRecursive) {
    for (var k in b) {
        if (isRecursive && a[k]) {
            var nodeA = exports.unwrap(a[k]).first.vals,
                nodeB = exports.unwrap(b[k]).first.vals;

            if (nodeA && nodeB) {
                a[k].nodes[0].vals = exports.merge(nodeA, nodeB, true);
            } else {
                a[k] = b[k];
            }
        } else {
