...
exports.lookupIndex = function(name, paths, filename){
  ...
  function lookupPackage(dir) {
    ...
    var packageInfo = require(relative(__dirname, package));
    var main = packageInfo.main;
    var style = packageInfo.style;
    if (/\.(css|styl)/.test(extname(main))) {
      found = exports.find(join(dir, main), paths, filename);
    }
    if (!found && /\.(css|styl)/.test(extname(style))) {
      found = exports.find(join(dir, style), paths, filename);
    }
    if (!found) {
