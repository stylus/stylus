...
exports.lookup = function(path, paths, ignore, prefix){
  
  // try with/without '_' prefix
  var lookup
    , i = paths.length;
    
  // add prefix to file basename
  if (prefix) {
    var filename = basename(path);
    path = path.substr(0, -filename.length) + prefix + filename;
  }
  ...
  // Try with prefix (recursive)
  if (!prefix) {
    return arguments.callee(path, paths, ignore, '_');
  }
};
