...
exports.formatException = function(renderer, err, options){
  var lineno = renderer.evaluator
    ...
    , filename = (renderer.evaluator && renderer.evaluator.importStack.length)
      ? renderer.evaluator.importStack.pop()
      : renderer.options.filename
    , context = options.context || 8
    , context = context / 2
    , lines = (err.str || renderer.str).split('\n')
