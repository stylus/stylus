 ...
 * Apply the plugins
 */
Renderer.prototype.usePlugins = function(){
...
/**
 * Apply the plugins then parse. `this.ast` is assigned to the parsing result.
 */
...
/**
 * Replace `this.ast` with an ast with all expression evaluated.
 */
Renderer.prototype.evaluate = function(){
  this.evaluator = new this.options.Evaluator(this.ast, this.options);
  this.nodes = nodes;
 ...
 * Normalize `this.ast`.
 */
Renderer.prototype.normalize = function(){
  this.normalizer = new Normalizer(this.ast, this.options);
  this.ast = this.normalizer.normalize();
}

/**
 * Compile `this.ast` to css
 *
 * @return {String} [css]
 */
Renderer.prototype.compile = function(){
  this.compiler = this.options.sourcemap
    ? new (require('./visitor/sourcemapper'))(this.ast, this.options)
    : new (require('./visitor/compiler'))(this.ast, this.options)
  var ret = this.compiler.compile();
  // expose sourcemap
  if (this.options.sourcemap) this.sourcemap = this.compiler.map.toJSON();
  return ret;
}
...
Renderer.prototype.render = function(fn){
  try {
    ...
    options.lineno = err.lineno || this.parser.lexer.lineno;
    options.column = err.column || this.parser.lexer.column;
...
Renderer.prototype.deps = function(filename){
  ...
  try {
    this.parse()
    var resolver = new DepsResolver(this.ast, opts);
